-----------------------------------------------------------------------------------------------
-- Client Lua Script for VikingCastBar
...
function VikingCastBar:Init()
	Apollo.RegisterAddon(self)
end

function VikingCastBar:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("VikingCastBar.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function VikingCastBar:OnDocumentReady()
...
function VikingCastBar:OnUpdate()
	...
	if unitPlayer:ShouldShowCastBar() then
		if bShowSimpleCast then
			...
			-- the sprite that fills the casting bar
				self.wndCastFrame:FindChild("CastingProgress"):SetFullSprite("BasicSprites:RedFill")
...
function VikingCastBar:DrawSingleBarFrame(wnd)
	...
	local strExtra = Apollo.GetString("VikingCastBar_Press")
	if Apollo.GetConsoleVariable("spell.useButtonDownForAbilities") then
		strExtra = Apollo.GetString("VikingCastBar_Hold")
	end
	
	wnd:FindChild("Label"):SetText(String_GetWeaselString(Apollo.GetString("VikingCastBar_ComplexLabel"), self.tCurrentOpSpell.strName, strExtra))
end

function VikingCastBar:DrawSingleBarFrameCircle(wnd)
	local fPercentDone = GameLib.GetSpellThresholdTimePrcntDone(self.tCurrentOpSpell.id)
	wnd:FindChild("Fill"):SetMax(1)
...
function VikingCastBar:DrawSingleBarFrameTiered(wnd)
	local fPercentDone = GameLib.GetSpellThresholdTimePrcntDone(self.tCurrentOpSpell.id)
	...
	wnd:FindChild("Label"):SetText(String_GetWeaselString(Apollo.GetString("VikingCastBar_ComplexLabel"), self.tCurrentOpSpell.strName, strExtra)) -- todo: Set the name once we have a function for it
end
...
function VikingCastBar:OnGenerateTooltip(wndHandler, wndControl, eType, spl)
...
function VikingCastBar:OnStartSpellThreshold(idSpell, nMaxThresholds, eCastMethod) -- also fires on tier change
...
function VikingCastBar:OnUpdateSpellThreshold(idSpell, nNewThreshold) -- Updates when P/H/R changes tier or RT tap is performed
...
function VikingCastBar:OnClearSpellThreshold(idSpell)
...
local CastBarInstance = VikingCastBar:new()
